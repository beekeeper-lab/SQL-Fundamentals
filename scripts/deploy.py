#!/usr/bin/env python3
"""Build and package the SQL Fundamentals course for student distribution.
...
Usage:
    ...
    uv run --with markdown --with pygments python scripts/deploy.py --version 1.2
...
Output:
    dist/sql-fundamentals-v{VERSION}.zip
...
def package_course(version: str, out_dir: Path) -> Path:
    ...
    # Name for the folder inside the zip (what students see when they unpack)
    folder_name = f"sql-fundamentals-v{version}"
    zip_stem = out_dir / folder_name

    # Create a staging directory with the right folder name
    ...
    # Create zip (shutil.make_archive wants the base name without .zip)
    zip_path = shutil.make_archive(
        str(zip_stem),       # base name for zip file
        "zip",               # format
        root_dir=str(staging),  # root for paths inside zip
        base_dir=folder_name,   # folder name inside zip
    )
