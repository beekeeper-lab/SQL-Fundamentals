...
Usage:
    ...
    uv run --with elevenlabs python scripts/generate_narration.py source/module-03-context-priming.md
    ...
    uv run --with elevenlabs python scripts/generate_narration.py source/module-03-context-priming.md --force
...
def main():
    ...
    parser.add_argument("source", nargs="?", help="Single source file (e.g. source/module-03-context-priming.md)")
    ...
    for f in files:
        ...
        total_generated += sum(1 for e in manifest if not Path(audio_dir / f.stem / e["audio_file"]).exists()) if args.dry_run else 0
